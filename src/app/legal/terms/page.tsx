  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-8 md:py-12">
      <nav className="text-sm text-[var(--ink-2)]">
        ...
        <span className="mx-2 text-[var(--rule)]">／</span>
        <span className="text-[var(--ink-3)]">利用規約</span>
      </nav>
      ...
      <article className="mt-6 space-y-6 text-sm leading-relaxed text-[var(--ink)]">
        <section>
          ...
          <p className="mt-2 text-[var(--ink-2)]">
            卓録（以下「本サービス」）は、TRPG卓・マダミス卓・人狼会・ボドゲ会等の「誰と・どれだけ続けたか」を本人が記録し、卓年鑑カードとして共有できる記録サービスです。
          </p>
        </section>
        ...
        <section>
          ...
          <p className="mt-2 text-[var(--ink-2)]">
            主催が明示的に公開を選択した場合に限り、卓年鑑カードはログイン無しで閲覧可能な単独URLで公開されます。公開には掲載されるメンバー一人ひとりの事前同意取得が必須です。主催はいつでも公開を停止できます。
          </p>
        </section>
        ...
        <section>
          ...
          <p className="mt-2 text-[var(--ink-2)]">
            ...
            で受け付けます。緊急性が高い案件は3営業日以内、それ以外は10営業日以内に一次対応します。虚偽通報は禁止です。
          </p>
        </section>
        ...
        <section>
          ...
          <p className="mt-2 text-[var(--ink-2)]">
            利用者は本サービス内から退会できます。退会時、登録された卓・メンバー・記録・公開リンクはすべて削除されます。退会前にデータのエクスポートが可能です。
          </p>
        </section>
